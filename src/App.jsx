...
// v296 Camada 3: componentes presentacionais.
import { AppIcon, IconText } from "./components/AppIcons.jsx";
import { EmptyState } from "./components/EmptyState.jsx";
import { Cd_ } from "./components/Cd.jsx";
...
// v295: "O que mudou" — aparece em modal quando o usuário loga com versão
// nova (diferente da última vista). Lista as melhorias mais visíveis.
// Quando bumpar versão, adicionar entrada nova no topo. Versões muito
// antigas podem ser podadas conforme tempo passa.
const WHATS_NEW={
  "v296-Xandroid":[
    "Botão GPS mostra spinner + texto \"Buscando satélite\" enquanto procura",
    "Setinhas nas bordas da barra de abas avisam quando há aba escondida",
    "Modal de recuperação de slot mostra thumbnail das primeiras 4 fotos",
    "GPS Início/Fim das trilhas com cores distintas (azul/laranja)",
    "Atalhos do PWA (long-press no ícone) agora funcionam: Nova cena, Foto, Exportar",
    "Versão centralizada em src/version.js (uma única edição em vez de 3)",
  ],
  "v295-Xandroid":[
    "10 slots de salvamento (antes eram 5)",
    "Tamanho de cada slot aparece na caixa",
    "Slots inativos apagam sozinhos após 30 dias",
    "Toast \"Nova versão disponível\" em vez de atualização silenciosa",
    "Modal \"O que mudou\" ao logar com versão nova",
    "DOCX agora suporta Sumário automático no Word (estilos Heading nomeados)",
    "Limite de 30 templates customizados (evita estouro de memória)",
    "Acessibilidade: imagens com descrições para leitor de tela",
  ],
  "v294-Xandroid":[
    "Confirmações em modais bonitos (antes alerta feio do navegador)",
    "BurstModal pergunta antes de descartar fotos pendentes",
    "Reverse geocoding pergunta antes de sobrescrever endereço já digitado",
    "Toast de erro fica visível até toque do usuário",
    "Bug do slot: ao sobrescrever, agora apaga o slot certo",
  ],
  "v293-Xandroid":[
    "Numeração dinâmica das seções no PDF e DOCX (não há mais buracos tipo \"4.3.1, 4.3.3\")",
    "GPS distingue erro de permissão / sem sinal / timeout com mensagens específicas",
  ],
  "v292-Xandroid":[
    "Cada dedo das mãos com IDs separados pra palma e dorso",
    "Bug do SUV \"fantasma\" corrigido (fallback quando vestígios não batem com categoria)",
    "Renderização do Croqui visual 3× mais rápida (paralelização)",
    "Timer ao vivo no toast ao gerar DOCX",
  ],
};
...
// Anatomia: imagens H/M, mãos, pés, regiões anatômicas, tipos de ferida
// Veículos: imagens, regiões clicáveis, posições no croqui visual, helpers
// ════════════════════════════════════════════════════════════════
import { IMG_H, IMG_M, IMG_MAO_D, IMG_MAO_E, IMG_PE_D, IMG_PE_E, bodyImgs, WT, AR } from "./constants/anatomy.js";
import { IMG_VEI, VEI_TIPOS_COM_SVG, VEHICLE_FIELDS, RVE, RVD, RVF, RVT, RVTe, RVI, AV, VVT, POS_VEI_LAT_E, POS_VEI_LAT_D, POS_VEI_FRENTE, POS_VEI_TRAS, POS_VEI_SUP, POS_VEI_INT_SUP, POS_VEI_INT_D, POS_VEI_INT_E, POS_MOTO_LAT, POS_MOTO_FT, POS_MOTO_SUP, POS_BICI_LAT, POS_BICI_FT, POS_BICI_SUP, POS_BUS_LAT_E, POS_BUS_LAT_D, POS_BUS_FT, POS_BUS_INT, hasVehicleData } from "./constants/vehicles.js";
...
const BurstModal=React.memo(function BurstModal({rk,onClose,onConfirm,utils}){
  ...
  // v294: confirm modal local — antes era window.confirm (feio em iOS PWA)
  ...
  const handleCancel=()=>{
    ...
    if(captured.length>0){setShowCancelConfirm(true);return;}
    onClose();
  };
  return(<div role="dialog" aria-modal="true" aria-label="Várias fotos seguidas" style={{position:"fixed",inset:0,background:"#0d1117",zIndex:9999,display:"flex",flexDirection:"column",color:"#fff"}}>
    ...
    {/* v294: confirm modal local (substitui window.confirm que ficava feio em iOS PWA) */}
    {showCancelConfirm&&<div role="dialog" aria-modal="true" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:10000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#1c1c1e",borderRadius:16,padding:24,maxWidth:360,width:"100%",boxShadow:"0 12px 40px rgba(0,0,0,0.5),0 0 0 0.5px rgba(255,255,255,0.08)"}}>
        <div style={{fontSize:36,textAlign:"center",marginBottom:12}}>⚠️</div>
        <div style={{fontSize:17,fontWeight:700,color:"#fff",textAlign:"center",marginBottom:8,letterSpacing:-0.3}}>Descartar fotos?</div>
        <p style={{fontSize:14,color:"rgba(255,255,255,0.75)",textAlign:"center",margin:"0 0 18px",lineHeight:1.5}}>Você vai perder <b>{captured.length} foto{captured.length>1?"s":""}</b> capturada{captured.length>1?"s":""}. Não dá pra desfazer.</p>
        <div style={{display:"flex",gap:10}}>
          <button type="button" style={{flex:1,padding:"13px 18px",borderRadius:12,background:"rgba(255,255,255,0.12)",color:"#fff",border:"1px solid rgba(255,255,255,0.18)",fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:"inherit",minHeight:48}} onClick={()=>setShowCancelConfirm(false)}>Continuar tirando</button>
          <button type="button" style={{flex:1,padding:"13px 18px",borderRadius:12,background:"#ff3b30",color:"#fff",border:"none",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",minHeight:48}} onClick={()=>{setShowCancelConfirm(false);onClose();}}>Descartar</button>
        </div>
      </div>
    </div>}
  </div>);
});
// Cd_ e EmptyState extraídos pra src/components/ (refactor v296 Camada 3)
...
// APP_ICONS, AppIcon, IconText extraídos pra src/components/AppIcons.jsx
// (refactor v296 Camada 3)
...
// v295: ouvinte do evento `xandroid:update-available` disparado pelo SW
// (em main.jsx) quando uma nova versão termina de baixar. Mostra banner
// de atualização disponível.
useEffect(()=>{const onUpdate=()=>setUpdateAvailable(true);window.addEventListener("xandroid:update-available",onUpdate);return()=>window.removeEventListener("xandroid:update-available",onUpdate);},[]);
...
// v296: Handler dos shortcuts do PWA (long-press no ícone do app abre menu).
// As URLs no manifest.webmanifest são ?action=new|capture|export — aqui
// detectamos e disparamos a ação após login (usuário não-logado vai
// pra tela de login normal).
useEffect(()=>{if(!loggedIn)return;
let action="";try{action=new URLSearchParams(window.location.search).get("action")||"";}catch(_){}
if(!action)return;
// Limpa o action da URL pra não disparar de novo no próximo render
try{const url=new URL(window.location.href);url.searchParams.delete("action");window.history.replaceState({},"",url.pathname+(url.search?"?"+url.searchParams.toString():"")+url.hash);}catch(_){}
if(action==="new"){setTimeout(()=>setShowTemplatePicker(true),200);showToast("✨ Atalho: Novo croqui");}
else if(action==="export"){setTab(TAB_EXPORTAR);showToast("📄 Atalho: Exportar");}
else if(action==="capture"){setTimeout(()=>{const inp=document.createElement("input");inp.type="file";inp.accept="image/*";inp.capture="environment";inp.onchange=async(e)=>{const fls=Array.from(e.target.files||[]);if(!fls.length)return;const legend=mkAutoLegend("solicitacao");const gps=await captureGPS();const novas=[];for(const fl of fls){try{const foto=await compressImg(fl);novas.push({id:uid(),ref:"solicitacao",desc:legend.desc,fase:"",local:legend.local,...foto,...(gps?{gps}:{})});}catch(eC){console.warn("CQ:",eC);}}if(novas.length){setFotos(p=>[...p,...novas]);haptic("heavy");showToast("📷 "+novas.length+" foto"+(novas.length>1?"s":""));}};inp.click();},300);}
...
// v295: estado para banner "Nova versão disponível" e modal "O que mudou"
const[updateAvailable,setUpdateAvailable]=useState(false);
const[whatsNewData,setWhatsNewData]=useState(null);// {versions:["v295-Xandroid","v294-Xandroid"]}
// v296: estado dos indicadores "tem mais aba" na barra de tabs
...
// v294: erros (❌ / ⚠) ficam VISÍVEIS até o usuário tocar pra fechar.
// Antes: todo toast sumia em 3s — erro passava despercebido se o usuário
// estava olhando pra outro lugar.
const[toast,setToast]=useState("");
const _toastTimerRef=useRef(null);
const showToast=(msg)=>{
  if(_toastTimerRef.current){clearTimeout(_toastTimerRef.current);_toastTimerRef.current=null;}
  setToast(msg);
  try{
    if(msg.startsWith("✅")||msg.includes("Salvo")||msg.includes("Copiado")||msg.includes("restaurado")||msg.includes("carregado"))haptic("success");
    else if(msg.startsWith("❌"))haptic("error");
    else if(msg.startsWith("⚠")||msg.includes("Atenção"))haptic("warning");
    else if(msg.startsWith("📷"))haptic("medium");
  }catch(e){/* haptic não suportado, ok */}
  // Erros e avisos ficam permanentes até toque do usuário
  const isErr=msg.startsWith("❌")||msg.startsWith("⚠");
  if(!isErr)_toastTimerRef.current=setTimeout(()=>{setToast("");_toastTimerRef.current=null;},3000);
};
const dismissToast=()=>{if(_toastTimerRef.current){clearTimeout(_toastTimerRef.current);_toastTimerRef.current=null;}setToast("");};
...
const reloadSlots=useCallback(async()=>{const found=[];for(let si=0;si<NUM_SLOTS;si++){try{const bd=await loadFullSlot("cq_"+loginMat+"_"+si);if(bd)found.push({slot:si,bd});}catch(e){console.warn("CQ:",e);}}setSavedSlots(found);},[loginMat,loadFullSlot]);
...
// Scan ALL slots — delete expired (>30 dias). v295: era 5 slots, agora NUM_SLOTS.
const found=[];
let expired=0;
const now=Date.now();
for(let si=0;si<NUM_SLOTS;si++){try{const bd=await loadFullSlot("cq_"+loginMat+"_"+si);if(bd){
...
else if(usedSlots.size>=NUM_SLOTS){/* todos cheios — abrir picker */setShowConfirmNovo(false);setShowSlotOverwrite(true);return;}
...
// v294: limpar o slot de DESTINO (nextSlot) — não o ativo. Se nextSlot===slotIdx,
// é o mesmo, sem mudança. Se forem diferentes, o bug antigo apagava o slot ativo
// (que devia ser preservado) em vez do alvo.
clearBackup(nextSlot);setTab(0);setResetKey(k=>k+1);setBackupStatus("🆕 Novo");setShowConfirmNovo(false);setShowStartMenu(false);if(keepLogin)setTimeout(()=>setShowTemplatePicker(true),100);};
...
// v295: "O que mudou" — compara última versão vista com a atual e mostra
// modal listando as melhorias de cada versão entre as duas. No primeiro
// login (sem registro de "última vista"), só marca a atual sem mostrar
// modal pra não atrapalhar quem está estreando.
(async()=>{try{const seen=await window.storage?.get("cq_last_seen_ver_"+trimMat);const lastSeen=seen?.value||"";if(!lastSeen){await window.storage?.set("cq_last_seen_ver_"+trimMat,APP_VERSION);return;}
if(lastSeen===APP_VERSION)return;
// Coleta todas as versões em WHATS_NEW que estão entre lastSeen (excl) e APP_VERSION (incl).
// Como WHATS_NEW está em ordem decrescente, percorremos até bater na lastSeen.
const versionsToShow=[];for(const ver of Object.keys(WHATS_NEW)){if(ver===lastSeen)break;versionsToShow.push(ver);}
if(versionsToShow.length){setWhatsNewData({versions:versionsToShow,trimMat});}
else{await window.storage?.set("cq_last_seen_ver_"+trimMat,APP_VERSION);}
}catch(e){console.warn("CQ whats new:",e);}})();};
...
// v295: limite de 30 templates customizados pra não detonar quota do iPhone.
// Sem limite, alguém pode salvar 200 templates e ficar com >50 MB de IDB ocupado.
const MAX_CUSTOM_TEMPLATES=30;
const saveCustomTemplate=async()=>{const name=(tplNameInput||"").trim();if(!name){showToast("⚠ Digite um nome");return;}
if(customTemplates.length>=MAX_CUSTOM_TEMPLATES){showToast(`⚠ Limite de ${MAX_CUSTOM_TEMPLATES} templates atingido — apague algum antes`);return;}
// Extrai só campos reutilizáveis dos dados atuais (exclui os que mudam por ocorrência)
...
// v294: clearBackup aceita slotIdx explícito pra evitar stale closure quando
// chamado logo após setSlotIdx (React faz batch — slotIdx do closure ainda
// está velho durante a execução de resetAll). Bug antigo: ao sobrescrever
// um slot DIFERENTE do ativo, o clearBackup apagava o slot ATIVO em vez
// do alvo (que era pra ser sobrescrito).
const clearBackup=async(slotOverride)=>{const idx=slotOverride!==undefined?slotOverride:slotIdx;try{await deleteFullSlot("cq_"+loginMat+"_"+idx);}catch(e){console.warn("CQ:",e);}setShowStartMenu(false);};
...
// v294: se já tem endereço preenchido, PERGUNTA antes de sobrescrever.
// Antes: pulava silencioso quando end estava preenchido — usuário podia
// querer atualizar e não tinha como sem apagar manualmente primeiro.
try{const ac=new AbortController();const tid=setTimeout(()=>ac.abort(),6000);fetch(`https://nominatim.openstreetmap.org/reverse?lat=${p.coords.latitude}&lon=${p.coords.longitude}&format=json&addressdetails=1`,{headers:{"Accept-Language":"pt-BR"},signal:ac.signal}).then(r=>{clearTimeout(tid);return r.json();}).then(j=>{
  if(!j||!j.display_name)return;
  const addr=j.display_name;
  const cur=(data.end||"").trim();
  if(!cur){s("end",addr);showToast("📍 Endereço: "+addr.slice(0,50)+"…");}
  else if(cur!==addr){
    reqDel(
      "Substituir endereço pelo do GPS?",
      `Atual: ${cur.slice(0,80)}${cur.length>80?"…":""}\n\nGPS: ${addr.slice(0,80)}${addr.length>80?"…":""}`,
      ()=>{s("end",addr);showToast("📍 Endereço atualizado");},
      {okLabel:"Substituir",cancelLabel:"Manter atual",danger:false}
    );
  }
}).catch(e=>{clearTimeout(tid);if(e.name!=="AbortError")console.warn("CQ geocoding:",e);});}catch(e){console.warn("CQ geocoding init:",e);}},
...
// v295: pStyle (Heading1/Heading2/Heading3) marca o parágrafo como título
// pra o Word reconhecer e gerar Sumário automático.
const pStyle=opts.pStyle?`<w:pStyle w:val="${opts.pStyle}"/>`:"";
const pPr=`<w:pPr>${pStyle}${keepNext}${keepLines}${center}${brd}${spacing}${ind}${shd?`<w:shd w:val="clear" w:color="auto" w:fill="${opts.shd}"/>`:""}<w:rPr>${bold}${italic}<w:sz w:val="${sz}"/>${color}</w:rPr></w:pPr>`;return`<w:p>${pPr}<w:r><w:rPr>${bold}${italic}${caps}<w:sz w:val="${sz}"/><w:szCs w:val="${sz}"/><w:rFonts w:ascii="Arial" w:hAnsi="Arial"/>${color}</w:rPr><w:t xml:space="preserve">${esc2(text)}</w:t></w:r></w:p>`;};
...
// v295: usa estilos nomeados Heading1/2/3 — Word reconhece como títulos
// e gera Sumário automático (Referências → Sumário).
const _secN={h1:0,h2:0,h3:0};
const H1=(title)=>{_secN.h1++;_secN.h2=0;_secN.h3=0;return Pp(`${_secN.h1} ${title}`,{bold:true,sz:28,caps:true,spBef:280,spAft:140,color:"1A1A2E",pStyle:"Heading1"});};
const H2=(title)=>{_secN.h2++;_secN.h3=0;return Pp(`${_secN.h1}.${_secN.h2} ${title}`,{bold:true,sz:26,spBef:200,spAft:140,color:"1A1A2E",pStyle:"Heading2"});};
const H3=(title)=>{_secN.h3++;return Pp(`${_secN.h1}.${_secN.h2}.${_secN.h3} ${title}`,{bold:true,italic:true,sz:24,spBef:140,spAft:100,color:"1A1A2E",pStyle:"Heading3"});};
...
// ──── FOTOGRAFIAS ────
// v295: ctXml inclui Override pra styles.xml (registra os estilos Heading
// 1/2/3 que o Word usa pra gerar Sumário automático).
let ctXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="jpeg" ContentType="image/jpeg"/><Default Extension="png" ContentType="image/png"/><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/><Override PartName="/word/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml"/><Override PartName="/word/header1.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.header+xml"/><Override PartName="/word/footer1.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.footer+xml"/></Types>';
...
// Rels
// v295: rIdStyles aponta pra word/styles.xml (estilos Heading nomeados)
let relsXml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rIdStyles" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/><Relationship Id="rIdHeader1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/header" Target="header1.xml"/><Relationship Id="rIdFooter1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer" Target="footer1.xml"/>';
...
// POS_VEI_*, POS_MOTO_*, POS_BICI_*, POS_BUS_* e VEI_TIPOS_COM_SVG agora
// vêm de ./constants/vehicles.js (refactor v296 — extraído pra arquivo próprio).
...
<div style={{display:"flex",flexWrap:"wrap",gap:6}}>{fotos.filter(f=>{if(!fotoFilter)return true;const tabMap={"solicitacao":TAB_SOLICITACAO,"local":TAB_LOCAL,"vestigios":TAB_VESTIGIOS,"cadaver":TAB_CADAVER,"veiculo":TAB_VEICULO};if(tabMap[fotoFilter]!==undefined)return fotoTab(f.ref)===tabMap[fotoFilter];return f.fase===fotoFilter;}).map((f,i)=><div key={f.id} style={{position:"relative",cursor:"pointer"}} onClick={()=>setEditFotoId(f.id)}><img src={f.dataUrl} alt={f.desc||"Foto sem descrição"} loading="lazy" style={{width:80,height:80,objectFit:"cover",borderRadius:8,border:`2px solid ${f.desc?t.ok:t.bd}`}}/><div style={{fontSize:8,color:t.t2,textAlign:"center",maxWidth:80,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.desc?.slice(0,15)||"sem desc"}</div></div>)}</div></Cd_>}
<Cd_ styles={ST} title={"Slots de Salvamento (Slot "+(slotIdx+1)+" ativo)"} icon="💾"><p style={{fontSize:12,color:t.t2,marginBottom:10,lineHeight:1.6}}>Você tem <b>{NUM_SLOTS} gavetas</b> para salvar croquis. O auto-save grava no slot ativo. Slots inativos são apagados após <b>30 dias</b>.</p><div style={{display:"flex",flexDirection:"column",gap:6}}>{Array.from({length:NUM_SLOTS},(_,si)=>si).map(si=>{const sd=getSlot(si);const isActive=slotIdx===si;
// v205: extrai miniatura do desenho (primeiro croqui)
...
// v295: tamanho aproximado do slot — soma o tamanho do JSON principal + fotos + desenho.
// Calcular byte-exact via Blob é caro pra cada render; tomamos o length da string (≈1 byte por char ASCII).
const slotKB=(()=>{if(!sd)return 0;let bytes=0;try{bytes+=JSON.stringify(sd).length;}catch(e){}return Math.round(bytes/1024);})();
const slotSizeStr=slotKB>=1024?(slotKB/1024).toFixed(1)+" MB":slotKB+" KB";
return(<div key={si} style={{background:isActive?(dark?"#1a2a1a":"#e8f5e9"):(dark?"#1c1c1e":"#f8f8fa"),border:`2px solid ${isActive?t.ok:t.bd}`,borderRadius:10,padding:12,display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>{/* Miniatura */}<div style={{flexShrink:0,width:64,height:46,borderRadius:6,background:dark?"#0a0a0a":"#fff",border:`1px solid ${t.bd}`,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>{thumb?<img src={thumb} alt="" aria-hidden="true" style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain"}} loading="lazy"/>:<span style={{fontSize:18,opacity:0.3}} aria-hidden="true">📋</span>}{sd&&sd.fotos&&sd.fotos.length>0&&<span style={{position:"absolute",bottom:1,right:2,fontSize:8,fontWeight:700,background:"rgba(0,122,255,0.9)",color:"#fff",padding:"1px 4px",borderRadius:6,lineHeight:1}}>📷{sd.fotos.length}</span>}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,color:isActive?t.ok:t.tx,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span>Slot {si+1} {isActive?"(ativo)":""}</span>{sd&&<span style={{fontSize:10,color:t.t2,fontWeight:600,background:dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.06)",padding:"1px 7px",borderRadius:6,fontVariantNumeric:"tabular-nums"}} title="Tamanho aproximado do slot">{slotSizeStr}</span>}</div>{sd?(<><p style={{fontSize:11,color:t.t2,margin:"4px 0 0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>Oc.: <b>{sd.data?.oc||"—"}/{(sd.data?.oc_ano||"").slice(-2)}</b> {sd.data?.dp||""} — {sd.data?.nat||""}</p><p style={{fontSize:10,color:t.t3,margin:"2px 0 0"}}>Salvo: {fmtDt(sd.timestamp)}</p></>):(<p style={{fontSize:11,color:t.t3,margin:"4px 0 0",fontStyle:"italic"}}>Vazio</p>)}</div><div style={{display:"flex",gap:4,flexShrink:0}}>{sd&&<button type="button" style={{...tb(false),fontSize:11,padding:"6px 10px"}} onClick={async()=>{try{const bd=await loadFullSlot("cq_"+loginMat+"_"+si);if(bd){setRecupData({...bd,_slotIdx:si});setShowConfirmRecup(true);}else{showToast("❌ Slot vazio");}}catch(e){showToast("❌ Erro");}}}><AppIcon name="📂" size={14} mr={4}/>Abrir</button>}{!sd&&!isActive&&<button type="button" style={{...tb(false),fontSize:11,padding:"6px 10px"}} onClick={()=>{setSlotIdx(si);showToast("✅ Slot "+(si+1)+" ativado!");}}><AppIcon name="✚" size={14} mr={4}/>Usar</button>}{sd&&<button type="button" style={{background:"rgba(255,59,48,0.12)",border:`1.5px solid ${t.no}`,color:t.no,cursor:"pointer",fontSize:18,fontWeight:700,borderRadius:8,padding:"4px 10px",minWidth:44,minHeight:44,lineHeight:1,fontFamily:"inherit"}} title="Apagar slot" aria-label="Apagar slot" onClick={async()=>{await deleteFullSlot("cq_"+loginMat+"_"+si);setSavedSlots(prev=>prev.filter(s=>s.slot!==si));if(isActive)resetAll(true);haptic("medium");showToast("🗑️ Slot "+(si+1)+" apagado");}}>×</button>}</div></div>);})}
...
.di-spinner{display:inline-block;width:14px;height:14px;border:2px solid rgba(255,255,255,0.32);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;flex-shrink:0}
...
<div style={{flexShrink:0,width:64,height:48,borderRadius:10,background:dark?"#0a0a0a":"#fff",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",boxShadow:"inset 0 0 0 1px rgba(0,0,0,0.06)"}}>{thumb?<img src={thumb} alt="" aria-hidden="true" style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain"}} loading="lazy"/>:<span style={{fontSize:20,opacity:0.4}} aria-hidden="true">📋</span>}{bd.fotos&&bd.fotos.length>0&&<span style={{position:"absolute",bottom:2,right:2,fontSize:9,fontWeight:800,background:t.ac,color:"#fff",padding:"1px 5px",borderRadius:8,lineHeight:1}}>{bd.fotos.length}</span>}</div>
...
// v294: erros são clicáveis (toque pra fechar) e ficam visíveis até o
// usuário descartar. Não-erros somem em 3s automaticamente.
return(<div className="di-toast" role={isErr?"alert":"status"} aria-live={isErr?"assertive":"polite"} onClick={isErr?dismissToast:undefined} style={{position:"fixed",top:"calc(env(safe-area-inset-top) + 8px)",left:"50%",transform:"translateX(-50%)",background:bg,color:"#fff",padding:"10px 18px",borderRadius:24,fontSize:14,fontWeight:600,letterSpacing:-0.2,zIndex:9999,boxShadow:"0 8px 24px rgba(0,0,0,0.35),0 0 0 0.5px rgba(255,255,255,0.15) inset",maxWidth:"90vw",textAlign:"center",pointerEvents:isErr?"auto":"none",cursor:isErr?"pointer":"default",display:"inline-flex",alignItems:"center",gap:8,whiteSpace:"normal",overflow:"hidden"}}><IconText text={toast} size={16}/>{isErr&&<span style={{fontSize:12,opacity:0.85,fontWeight:500,whiteSpace:"nowrap"}}>· toque pra fechar ✕</span>}</div>);})()}
...
{/* v295: banner "Nova versão disponível" — disparado pelo SW quando termina de baixar */}
{updateAvailable&&!photoSaveStuck&&<div role="status" aria-live="polite" style={{position:"fixed",top:"calc(env(safe-area-inset-top) + 56px)",left:8,right:8,zIndex:9997,background:"linear-gradient(180deg,#0a84ff,#0066cc)",color:"#fff",padding:"10px 14px",borderRadius:14,fontSize:13,fontWeight:600,letterSpacing:-0.2,boxShadow:"0 4px 14px rgba(10,132,255,0.40),inset 0 1px 0 rgba(255,255,255,0.18)",display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20,flexShrink:0}} aria-hidden="true">✨</span><span style={{flex:1,minWidth:0}}>Nova versão disponível — <b>recarregue para aplicar</b>.</span><button type="button" onClick={()=>window.location.reload()} style={{background:"rgba(255,255,255,0.25)",color:"#fff",border:"1px solid rgba(255,255,255,0.4)",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>Recarregar</button><button type="button" aria-label="Fechar" onClick={()=>setUpdateAvailable(false)} style={{background:"transparent",color:"#fff",border:"none",fontSize:18,cursor:"pointer",padding:"4px 6px",fontFamily:"inherit",lineHeight:1,flexShrink:0,opacity:0.8}}>×</button></div>}
{/* v295: modal "O que mudou" — aparece ao logar com versão nova */}
{whatsNewData&&<div role="dialog" aria-modal="true" className="modal-overlay" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:2400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
<div className="modal-box" style={{background:t.cd,borderRadius:18,padding:24,maxWidth:480,width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:"0 12px 40px rgba(0,0,0,0.4)"}}>
<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{fontSize:32}} aria-hidden="true">✨</span><div><div style={{fontSize:18,fontWeight:800,color:t.tx,letterSpacing:-0.3}}>O que mudou</div><div style={{fontSize:11,color:t.t2}}>Versão atual: {APP_VERSION}</div></div></div>
<p style={{fontSize:12,color:t.t2,margin:"0 0 14px",lineHeight:1.5}}>Atualizamos o app desde a última vez que você usou. Aqui estão as melhorias:</p>
<div style={{flex:1,overflowY:"auto",paddingRight:6}}>
{whatsNewData.versions.map(ver=>{const items=WHATS_NEW[ver]||[];if(!items.length)return null;return(<div key={ver} style={{marginBottom:18}}>
<div style={{fontSize:13,fontWeight:700,color:t.ac,marginBottom:8,letterSpacing:0.3,textTransform:"uppercase"}}>{ver}</div>
<ul style={{margin:0,paddingLeft:20,fontSize:13,color:t.tx,lineHeight:1.6}}>{items.map((item,i)=><li key={i} style={{marginBottom:4}}>{item}</li>)}</ul>
</div>);})}
</div>
<button type="button" style={{...bt,background:t.ac,color:"#fff",width:"100%",textAlign:"center",padding:"14px",fontSize:15,fontWeight:700,marginTop:16}} onClick={async()=>{try{await window.storage?.set("cq_last_seen_ver_"+(whatsNewData.trimMat||loginMat),APP_VERSION);}catch(e){console.warn("CQ ver mark:",e);}setWhatsNewData(null);haptic("medium");}}><AppIcon name="✓" size={14} mr={6}/>Entendi</button>
</div>
</div>}
...
<div style={{textAlign:"center",marginBottom:12}}><img src={ef.dataUrl} alt={ef.desc||"Foto sem descrição"} style={{maxWidth:"100%",maxHeight:200,borderRadius:10,border:`2px solid ${t.ac}`}}/><div style={{fontSize:10,color:t.t3,marginTop:4}}>{ef.w}×{ef.h} | {ef.sizeKB}KB | {ef.name||""}</div></div>
...
{showConfirmRecup&&recupData&&(()=>{
// v296: modal de recuperação mostra thumbnail das primeiras 4 fotos do slot
const fotosPreview=Array.isArray(recupData.fotos)?recupData.fotos.slice(0,4):[];
const fotosRest=Math.max(0,(recupData.fotos?.length||0)-fotosPreview.length);
return(<div role="dialog" aria-modal="true" className="modal-overlay" style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:2100,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"12px 16px"}}><div className="modal-box" style={{background:t.cd,borderRadius:16,padding:24,maxWidth:420,width:"100%",maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,0.3)"}}><div style={{fontSize:16,fontWeight:700,color:t.tx,marginBottom:8}}>📂 Slot {(recupData._slotIdx!==null?recupData._slotIdx:slotIdx)+1}</div><p style={{fontSize:13,color:t.t2,marginBottom:4}}>Salvo em: {fmtDt(recupData.timestamp)}</p>{recupData.data?.oc&&<p style={{fontSize:13,fontWeight:600,color:t.ac,marginBottom:4}}>Oc.: {recupData.data.oc}/{recupData.data.oc_ano||""} {recupData.data.dp||""}</p>}{recupData.data?.nat&&<p style={{fontSize:12,color:t.t2,marginBottom:4}}>Natureza: {recupData.data.nat}</p>}<div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:12,fontSize:11,color:t.t2}}>{recupData.vestigios&&<span>🧪 {recupData.vestigios.filter(v=>v.desc).length} vest.</span>}{recupData.fotos&&<span>📷 {recupData.fotos.length} fotos</span>}{recupData.data?.end&&<span>📍 {recupData.data.end.slice(0,30)}</span>}</div>
{fotosPreview.length>0&&<div style={{display:"flex",gap:6,marginBottom:14,padding:8,background:t.bg3,borderRadius:10}}>{fotosPreview.map((f,i)=><img key={i} src={f.dataUrl} alt={f.desc||"Foto "+(i+1)} loading="lazy" style={{width:64,height:64,objectFit:"cover",borderRadius:8,border:`1.5px solid ${t.bd}`,flexShrink:0}}/>)}{fotosRest>0&&<div style={{width:64,height:64,borderRadius:8,background:dark?"rgba(10,132,255,0.18)":"rgba(0,122,255,0.10)",border:`1.5px dashed ${t.ac}`,display:"flex",alignItems:"center",justifyContent:"center",color:t.ac,fontWeight:700,fontSize:13,flexShrink:0,letterSpacing:-0.3}}>+{fotosRest}</div>}</div>}
<p style={{fontSize:12,color:t.no,fontWeight:600,marginBottom:16}}>Isso substituirá todos os dados atuais.</p><div style={{display:"flex",gap:10}}><button type="button" style={{...bt,background:t.ac,color:"#fff",flex:1,textAlign:"center",padding:"14px"}} onClick={()=>{const bd=recupData;const si=bd._slotIdx!==null?bd._slotIdx:slotIdx;applyBackupData(bd);setSlotIdx(si);setShowConfirmRecup(false);setRecupData(null);showToast("✅ Slot "+(si+1)+" carregado!");}}><AppIcon name="✓" size={14} mr={4}/>Carregar</button><button type="button" style={{...bt,background:t.bg3,color:t.tx,border:`1px solid ${t.bd}`,flex:1,textAlign:"center",padding:"14px"}} onClick={()=>{setShowConfirmRecup(false);setRecupData(null);}}>Cancelar</button></div></div></div>);
})()}
{/* v268: modal de escolha de slot quando todos cheios. v295: NUM_SLOTS=10 */}
...
<p style={{fontSize:12,color:t.t2,marginBottom:14,lineHeight:1.5}}>Você tem os {NUM_SLOTS} slots de auto-save ocupados. Escolha qual <b>sobrescrever</b> com o novo croqui — o conteúdo desse slot será <b>perdido permanentemente</b>. Faça backup JSON antes se precisar.</p>
<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
{Array.from({length:NUM_SLOTS},(_,si)=>si).map(si=>{const sl=savedSlots.find(s=>s.slot===si);const isCurrent=si===slotIdx;const oc=sl?.data?.oc;const dp=sl?.data?.dp;const nat=sl?.data?.nat;const ts=sl?.timestamp;return(
...
{/* v296: indicadores "tem mais aba" nas bordas da barra de tabs */}
{tabsOverflow.left&&<div aria-hidden="true" style={{position:"fixed",top:headerH.top,left:0,height:50,width:32,zIndex:1002,pointerEvents:"none",background:dark?"linear-gradient(90deg,rgba(0,0,0,0.85),rgba(0,0,0,0))":"linear-gradient(90deg,rgba(255,255,255,0.92),rgba(255,255,255,0))",display:"flex",alignItems:"center",justifyContent:"flex-start",paddingLeft:4}}><span style={{fontSize:18,color:t.ac,fontWeight:700,filter:`drop-shadow(0 0 4px ${t.ac}66)`}}>‹</span></div>}
{tabsOverflow.right&&<div aria-hidden="true" style={{position:"fixed",top:headerH.top,right:0,height:50,width:32,zIndex:1002,pointerEvents:"none",background:dark?"linear-gradient(270deg,rgba(0,0,0,0.85),rgba(0,0,0,0))":"linear-gradient(270deg,rgba(255,255,255,0.92),rgba(255,255,255,0))",display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:4}}><span style={{fontSize:18,color:t.ac,fontWeight:700,filter:`drop-shadow(0 0 4px ${t.ac}66)`}}>›</span></div>}
